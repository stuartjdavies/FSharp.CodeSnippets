...
let xs = DateTime.ParseExact("01/05/2014 14:50:06", format, System.Globalization.CultureInfo.InvariantCulture)
...
let trans = seq { 1 .. reader.NumberOfPages } 
            ...
            |> Seq.map(fun (ln : string) -> let tokens = ln.Split(' ')                                           
                                            ...
                                            { 
                                                MikiTransaction.Date = DateTime.ParseExact(String.Format("{0} {1}",tokens.[0], tokens.[1]), """dd/MM/yyyy HH:mm:ss""", System.Globalization.CultureInfo.InvariantCulture)                                                                                            
...
let myki = Frame.ofRecords trans // |> Frame.indexRowsDate "Date" 

let dates = myki.GetColumn<DateTime>("Date") |> Series.values
let debits = myki.GetColumn<Single>("Debit") |> Series.values

myki.GetColumn<Single>("Debit") |> Stats.count
myki.GetColumn<Single>("Debit") |> Stats.max
myki.GetColumn<Single>("Debit") |> Stats.min

Seq.zip dates debits |> Chart.Point
